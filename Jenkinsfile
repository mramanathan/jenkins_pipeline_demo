        ...
        stage('GitHub-Token') {
            steps {
                ws("${WORKSPACE}/github_token") {
                    script {
                        withCredentials([string(credentialsId: 'c12d52bc-6e8d-4604-a663-7fc0cfbeadd5', variable: 'GH_TOKEN')]) {
                            sh "git clone https://${GH_TOKEN}@github.com/mramanathan/jenkins_pipeline_demo.git --branch github_token"
