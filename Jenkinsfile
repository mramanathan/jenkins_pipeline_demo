                ...
                ws("${env.BRANCH_NAME}/token") {
                    script {
                        withCredentials([string(credentialsId: 'c12d52bc-6e8d-4604-a663-7fc0cfbeadd5', variable: 'GHE_TOKEN')]) {
                            ...
                            env.gitCommit  = sh(returnStdout: true, script: 'git rev-parse --verify HEAD').trim()
                            println "Latest commit's SHA1 id, ${gitCommit}"
                            env.remoteRepo = sh(returnStdout: true, script: 'git config --get remote.origin.url').trim()
